...
//
// Three exits write a shut operating point and they seat the collar at three
// different potentials. The question is whether each seat is a NEW object or one
...
//
// The answer is a count: over a wide sweep every hydraulic shutdown seats the
// collar at one of two REGISTERED INPUTS, the searched dry bound is the seat
...
//
//   make CXX=g++ probe_shut_seat && stdbuf -oL ./probe_shut_seat
...
struct Tally {
  ...
  long seat_is_psi_crit = 0, seat_is_root_psi_crit = 0, seat_is_neither = 0;
  long seat_is_dry_searched = 0, seat_is_wet_bound = 0;
...
void tally_one(Tally& t, double psi_soil, double ppfd, int layers,
  ...
  const double seat = l.opt_root_psi_;
  bool named = false;
  if (seat == l.psi_crit) { t.seat_is_psi_crit++; named = true; }
  if (seat == l.supply_psi_crit()) { t.seat_is_root_psi_crit++; named = true; }
  if (!named) {
    // Is it one of the two bounds the machinery already computes?
    try {
      if (seat == l.bound_row(Which::DryRootCrit).bound) {
        t.seat_is_dry_searched++; named = true;
      }
    } catch (const std::exception&) {}
    bool wet_refused = false;
    try {
      if (seat == l.bound_row(Which::Wet).bound) {
        t.seat_is_wet_bound++; named = true;
      }
  ...
  if (!named) {
    ...
    std::printf("  unnamed seat: %s psi_soil=%.3f ppfd=%.0f L=%d psi_crit=%.6f "
                "root_psi_crit=%.6f seat=%.17g root_crit=%.17g match=%d\n",
                phylloptim::Leaf::operating_point_kind_name(k), psi_soil, ppfd,
                layers, l.psi_crit, l.supply_psi_crit(), seat, rc,
                int(seat == rc));
  }
...
int main() {
  ...
  std::printf("\nwhere the seat already is, over %ld points:\n", n);
  std::printf("  == psi_crit (a registered input)          %ld\n", t.seat_is_psi_crit);
  std::printf("  == root_psi_crit (a registered input)     %ld\n", t.seat_is_root_psi_crit);
  std::printf("  == bound_row(DryRootCrit).bound           %ld\n", t.seat_is_dry_searched);
  std::printf("  == bound_row(Wet).bound                   %ld\n", t.seat_is_wet_bound);
  std::printf("  none of the four  ---------------------->  %ld", t.seat_is_neither);
  if (t.seat_is_neither)
    std::printf("   (of which the wet bound REFUSED at %ld; one seat: %.17g)",
